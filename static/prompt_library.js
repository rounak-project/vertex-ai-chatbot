...
document.querySelector("#improvePrompt").onclick=async()=>{ statusEl.textContent="Improving prompt"; const res=await fetch("/api/prompts/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:document.querySelector("#roughPrompt").value})}); const data=await res.json(); editor.value=data.prompt; statusEl.textContent=data.source==="groq"?"Improved with Groq":"Improved locally"; };
...
fetch("/api/prompts").then((r)=>r.json()).then((data)=>{ seedPrompts=data.prompts||[]; render(); renderRecent(); });
