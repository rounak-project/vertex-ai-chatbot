...
runButton.onclick=async()=>{ setStatus("Working"); runButton.disabled=true; try{ const endpoint=`/api/coding/${actionEl.value}`; const res=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:promptEl.value,code:editor.value,files,language:languageEl.value,target_language:targetLanguageEl.value})}); const data=await res.json(); if(data.files && Object.keys(data.files).length) loadFiles(data.files); responseEl.textContent=[data.message,data.explanation,(data.warnings||[]).join("\n")].filter(Boolean).join("\n\n"); setStatus(data.source==="groq"?"Groq response":"Local fallback"); }catch(error){ responseEl.textContent=error.message; setStatus("Error"); }finally{ runButton.disabled=false; } };
renderEditor();
